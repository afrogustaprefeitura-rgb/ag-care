...
async function saveAction(e,id,clienteId){e.preventDefault();const status=document.getElementById('fstatus').value;const payload={cliente_id:clienteId,acao:document.getElementById('facao').value.trim(),area:document.getElementById('farea').value.trim(),problema:document.getElementById('fproblema').value.trim()||null,prazo:document.getElementById('fprazo').value,responsavel:document.getElementById('fresp').value.trim()||null,prioridade:document.getElementById('fprior').value,fase_plano:document.getElementById('ffase').value,status,origem:'MANUAL',origem_id:null,concluido_em:status==='concluída'?new Date().toISOString():null};const r=id?await db.from('acoes').update(payload).eq('id',id):await db.from('acoes').insert(payload);if(r.error)return alert(r.error.message);await adminClient(clienteId);alert(id?'Ação atualizada.':'Ação criada.');}
