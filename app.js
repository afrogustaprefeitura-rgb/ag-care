...
async function client(){const r=await db.from('acoes').select('*').eq('cliente_id',S.client.id).order('prazo');if(r.error)return errorBox(r.error.message);S.actions=r.data||[];const all=S.actions,done=all.filter(x=>x.concluido_em!==null),open=all.filter(x=>x.concluido_em===null);const today=new Date();today.setHours(0,0,0,0);const in7=new Date(today);in7.setDate(in7.getDate()+7);const overdue=open.filter(x=>x.prazo&&new Date(x.prazo+'T00:00:00')<today),next7=open.filter(x=>x.prazo&&new Date(x.prazo+'T00:00:00')>=today&&new Date(x.prazo+'T00:00:00')<=in7);const pct=all.length?Math.round(done.length/all.length*100):0,avg=all.length?Math.round(all.reduce((n,x)=>n+(Number(x.score)||0),0)/all.length):0,priority=all.filter(x=>Number(x.score)>=70).length,inprog=open.filter(x=>x.status==='em andamento').length,blocked=open.filter(x=>x.status==='bloqueada').length;const p1=all.filter(x=>x.fase_plano==='0–30 dias'),p2=all.filter(x=>x.fase_plano==='31–60 dias'),p3=all.filter(x=>x.fase_plano==='61–90 dias');const h=await db.from('acoes_historico').select('id,acao_id,evento,dados,created_at').eq('cliente_id',S.client.id).order('created_at',{ascending:false}).limit(8);const history=h.error?[]:(h.data||[]);shell('Painel da empresa',`<section class="hero"><small>ACOMPANHAMENTO AG CARE · ${esc(S.client.id_diagnostico)}</small><h2>${esc(S.client.nome)}</h2><p>${esc(S.client.segmento)} · ${esc(S.client.cidade_uf)}</p></section><div class="grid client-kpis"><div class="card"><span>Plano concluído</span><b>${pct}%</b><small>${done.length} de ${all.length} ações</small></div><div class="card"><span>Ações atrasadas</span><b>${overdue.length}</b><small>${overdue.length?'Precisam de atenção':'Nenhuma ação atrasada'}</small></div><div class="card"><span>Próximos 7 dias</span><b>${next7.length}</b><small>${next7.length?'Com prazo nesta janela':'Nenhuma ação nesta janela'}</small></div><div class="card"><span>Score médio do plano</span><b>${avg}</b><small>${priority} ações com score ≥ 70</small></div></div>${a?`<section class="panel client-progress"><small>PLANO DE AÇÃO</small><h2>Progresso do plano</h2><p><b>${done.length} de ${all.length}</b> ações concluídas · ${pct}%</p><div class="progress-track"><span style="width:${pct}%"></span></div></section><section class="panel"><small>PLANO DE AÇÃO</small><h2>Progresso por etapa</h2><p>0–30 dias: <b>${p1.filter(x=>x.concluido_em).length}/${p1.length}</b> · 31–60 dias: <b>${p2.filter(x=>x.concluido_em).length}/${p2.length}</b> · 61–90 dias: <b>${p3.filter(x=>x.concluido_em).length}/${p3.length}</b></p></section>`:'<section class="success">🎉 Nenhuma ação aberta.</section>'}${table(all)}<section class="panel"><small>INDICADORES</small><h2>Visão do diagnóstico</h2><p>Score médio do plano <b>${avg}</b> · Ações prioritárias <b>${priority}</b> · Em andamento <b>${inprog}</b> · Bloqueadas <b>${blocked}</b></p></section><section class="panel"><small>MOVIMENTAÇÕES</small><h2>Últimas movimentações</h2>${history.length?history.map(x=>`<div class="movement"><b>${esc(x.evento==='atualizada'?'Ação atualizada':x.evento==='reaberta'?'Ação reaberta':x.evento==='concluida'?'Ação concluída':x.evento)}</b> · ${new Date(x.created_at).toLocaleString('pt-BR')}<p>${esc(x.dados?.depois?.acao||x.dados?.antes?.acao||'Movimentação registrada')}</p></div>`).join(''):'<p>Nenhuma movimentação registrada.</p>'}</section>`)}
