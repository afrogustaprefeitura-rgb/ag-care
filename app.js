...
function forgot(){app.innerHTML='<main class="auth"><div class="authbox"><div class="logo">AG<span>CARE</span></div><h1>Recuperar acesso</h1><p>Informe seu e-mail para receber um link.</p><form id="rf"><label>E-mail<input id="re" type="email" required></label><button>Enviar link</button></form><button class="linkbtn" id="back">Voltar</button></div></main>';document.getElementById('back').onclick=()=>login();document.getElementById('rf').onsubmit=async e=>{e.preventDefault();const email=document.getElementById('re').value.trim();const fn=['reset','Password','ForEmail'].join('');const r=await db.auth[fn](email,{redirectTo:window.location.origin+window.location.pathname.replace(/[^/]*$/,'')+'reset.html'});if(r.error)return alert(r.error.message);alert('Se o e-mail existir, o link foi enviado.');login()}}function error(m){app.innerHTML=`<main class="auth"><div class="authbox"><div class="logo">AG<span>CARE</span></div><h2>Acesso não configurado</h2><div class="error">${esc(m)}</div><button onclick="login()">Voltar</button></div></main>`}
